...
class Drone:
    def __init__(self, coordinate, name, host, port):
        self.coordinate = coordinate
        self.name = name
        self.endpoint = f"http://{host}:{port}/set_command"
        self.emergency = f"http://{host}:{port}/emergency"
        self.token = ''
...
def sign_up():
    ...
    try:
        drone = Drone(content['coordinate'], content['name'], content["host"], content["port"])
        drones.append(drone)
...
def main():
    """ Запуск системы организации воздушного движения. """
