...
def handle_event(id, details_str):
    ...
    if operation == "check":
        print("[HEALTH_CHECK_DEBUG] Checking battery")
        return proceed_to_deliver(id, {
            "deliver_to": "battery",
            "operation": "get_battery"
        })

    elif operation == "set_battery":
        battery = details.get("battery")
        if not isinstance(battery, int) or battery < 0 or battery > 100:
            print("[HEALTH_CHECK_DEBUG] Invalid battery data")
            battery = 0

        print("[HEALTH_CHECK_DEBUG] Battery:", battery)
